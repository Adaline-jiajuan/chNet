...
#' @usage chNet (X,group,subsampling,R,lambar,parallel,nCpus)
...
#' @param subsampling  Logical value to indicate if the process should be run to obtain a weight matrix, default to FALSE.
#' @param R The number of sub-sampled datasets; default is 20.
...
chNet <- function(X, group,subsampling = FALSE, R= 20, lambar, parallel = FALSE, nCpus = 4){
  ...
  if(subsampling ){
    ...
    weight.temp = matrix(0,p,p)

    for (ii in 1: R){

      ind_group1 = sample(ind1,0.8*len1)

      ind_group2 = sample(ind2,0.8*len2)

      group_temp = rbind(ind_group1, ind_group2)
      subgroup = group[group_temp]
      xsub = X[group_temp,]

      cc.subsam = compute.statis(xsub, subgroup, parallel, nCpus)
      lams.edge.subsam <- get.knots(cc.subsam$tmu, cc.subsam$trho)$int.knots

      lams.edge.weight <- (lams.edge.subsam >= lambar)*1
      diag(lams.edge.weight)=0
